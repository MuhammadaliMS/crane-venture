...
import { Lock, Mail, CheckCircle2, Check, AlertCircle, Circle } from 'lucide-react';
...
export function FounderForm() {
  ...
  function setCellValue(rowKey: string, monthKey: string, value: string) {
    setCellValues((prev) => ({ ...prev, [cellKey(rowKey, monthKey)]: value }));
  }
  ...
  // --- Render helpers ---
  function renderStatusIcon(row: RowDef) {
    const st = getRowStatus(row);
    if (st === 'complete') return <Check className="w-4 h-4 text-emerald-500" />;
    if (st === 'partial') return <AlertCircle className="w-4 h-4 text-amber-500" />;
  ...
  function renderRow(row: RowDef, rowIndex: number) {
    const lastKnown = getLastKnown(row);
    ...
    return (
      <tr key={row.key} className={isEven ? 'bg-white' : 'bg-slate-50/60'}>
        ...
        {quarter.months.map((month) => {
          ...
          return (
            <td key={month.key} className="px-0.5 py-0.5 border-b border-slate-100 min-w-[120px]">
              <input
                type="text"
                inputMode="decimal"
                value={val}
                ...
                className="w-full border border-slate-200 rounded px-2 py-1.5 text-right font-mono text-[13px] text-slate-800 placeholder:text-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-300 transition-shadow min-h-[44px]"
              />
            </td>
          );
        })}
        ...
        {/* Status */}
        <td className="px-2 py-1.5 text-center border-b border-slate-100 min-w-[50px]">
            ...
            <table className="w-full border-collapse min-w-[750px]">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  ...
                  <th className="px-2 py-2.5 text-center text-xs font-semibold text-slate-400 uppercase tracking-wider min-w-[50px]">
                    Status
                  </th>
                </tr>
              </thead>
