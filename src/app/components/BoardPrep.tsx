...
function SourceTag({ label }: { label: string }) {
  return (
    <span className="text-[10px] text-muted-foreground/70 bg-muted/60 px-1.5 py-0.5 rounded inline-flex items-center gap-0.5 whitespace-nowrap">
      Source: {label}
    </span>
  );
}
...
function DeltaArrow({ from, to, unit = '', invert = false }: { from: number; to: number; unit?: string; invert?: boolean }) {
  ...
  const color = diff === 0 ? 'text-muted-foreground/70' : isPositive ? 'text-emerald-600' : 'text-red-500';
  const Icon = diff === 0 ? Minus : isUp ? ArrowUpRight : ArrowDownRight;
  return (
    <div className="flex items-center gap-1 text-[12px]">
      <span className="text-muted-foreground/70">{from}{unit}</span>
...
function SectionHeader({ title, expanded, onClick, borderColor, icon }: {
  ...
  return (
    <button onClick={onClick} className="w-full flex items-center gap-2 p-4 hover:bg-muted/60 transition-colors">
      {expanded ? <ChevronDown className="w-4 h-4 text-muted-foreground/70" /> : <ChevronRight className="w-4 h-4 text-muted-foreground/70" />}
      {icon}
  ...
  return (
    <div className="max-w-[1400px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-5">
        <h1 className="text-[24px] font-semibold tracking-tight text-foreground">Board Prep</h1>
        <div className="flex items-center gap-3">
          <select
            value={selectedCompany}
            onChange={e => setSelectedCompany(e.target.value)}
            className="text-[13px] border border-border rounded-lg px-3 py-1.5 bg-white text-foreground focus:outline-none focus:ring-2 focus:ring-blue-100"
          >
          ...
          <button className="text-[12px] px-3 py-1.5 border border-border rounded-lg hover:bg-muted/60 flex items-center gap-1.5 text-foreground transition-colors">
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <div className="flex items-center gap-2 text-[12px] text-muted-foreground">
          ...
          <span className="text-[11px] bg-muted text-foreground/80 px-2 py-0.5 rounded">Last refreshed: 2 hours ago</span>
        </div>
      </div>
      ...
      <div className="flex gap-5">
            ...
            {expandedSections.has('context') && (
              <div className="px-5 pb-5 pt-0 space-y-4 animate-fade-in-up">
                ...
                <div className="flex items-center gap-4 flex-wrap">
                  ...
                  <span className="text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground/70 bg-muted/60 px-2 py-0.5 rounded">
                    Board #{boardNumber}
                  </span>
                ...
                <div className="bg-muted/60 rounded-lg p-3.5">
                  <p className="text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground/70 mb-2">Last Board — {lastBoardDate.toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                ...
                {/* Board members */}
                <div>
                  <p className="text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground/70 mb-2">Board Attendees</p>
                  <div className="flex flex-wrap gap-2">
                    {boardMembers.map((m, i) => (
                      <div key={i} className="flex items-center gap-2 bg-muted/60 rounded-lg px-3 py-1.5">
                        <div className="w-6 h-6 rounded-full bg-secondary flex items-center justify-center text-[10px] font-semibold text-foreground/80">
                          {m.name.charAt(0)}
                        </div>
                        <div>
                          <p className="text-[12px] font-medium text-foreground">{m.name}</p>
                          <p className="text-[10px] text-muted-foreground/70">{m.role}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="px-5 pb-5 pt-0 space-y-3 animate-fade-in-up">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="bg-muted/60 rounded-lg p-3">
                    <p className="text-[11px] text-muted-foreground/70 mb-1">Health Status</p>
                    <div className="flex items-center gap-1.5">
                      <div className="w-2.5 h-2.5 rounded-full" style={{ background: getHealthColor(company.health) }} />
                      <span className="text-[14px] font-medium text-foreground">{company.health}</span>
                    </div>
                    <SourceTag label="Calculated" />
                  </div>
                  {/* RAG */}
                  <div className="bg-muted/60 rounded-lg p-3">
                    <p className="text-[11px] text-muted-foreground/70 mb-1">RAG Status</p>
                    <div className="flex items-center gap-2">
                      <span className="text-[14px] font-medium text-foreground">{company.rag}</span>
                  ...
                  {/* MRR */}
                  <div className="bg-muted/60 rounded-lg p-3">
                    <p className="text-[11px] text-muted-foreground/70 mb-1">MRR</p>
                    <p className="text-[14px] font-mono-num font-medium text-foreground">{formatCurrency(company.mrr, company.currency)}</p>
                    <p className="text-[11px] text-emerald-600">+{company.arrGrowth}% MoM</p>
                    <SourceTag label="Board deck" />
                  </div>
                  {/* ARR */}
                  <div className="bg-muted/60 rounded-lg p-3">
                    <p className="text-[11px] text-muted-foreground/70 mb-1">ARR</p>
                    <p className="text-[14px] font-mono-num font-medium text-foreground">{formatCurrency(company.mrr * 12, company.currency)}</p>
                    <SourceTag label="Calculated" />
                  </div>
                  {/* Runway */}
                  <div className="bg-muted/60 rounded-lg p-3">
                    <p className="text-[11px] text-muted-foreground/70 mb-1">Runway</p>
                    <p className={`text-[14px] font-mono-num font-medium ${company.runway < 6 ? 'text-red-600' : 'text-foreground'}`}>{company.runway} months</p>
                  ...
                  {/* Customers */}
                  <div className="bg-muted/60 rounded-lg p-3">
                    <p className="text-[11px] text-muted-foreground/70 mb-1">Customers</p>
                    <p className="text-[14px] font-mono-num font-medium text-foreground">{company.customers}</p>
                ...
                <div className="bg-blue-50/60 border border-blue-200/40 rounded-lg p-3.5">
                  ...
                  <div className="grid grid-cols-5 gap-3">
                    <div>
                      <p className="text-[10px] text-muted-foreground/70 mb-0.5">MRR</p>
                      <DeltaArrow from={Math.round(prevMRR / 1000)} to={Math.round(company.mrr / 1000)} unit="K" />
                    </div>
                    <div>
                      <p className="text-[10px] text-muted-foreground/70 mb-0.5">Runway</p>
                      <DeltaArrow from={prevRunway} to={company.runway} unit="mo" invert />
                    </div>
                    <div>
                      <p className="text-[10px] text-muted-foreground/70 mb-0.5">Customers</p>
                      <DeltaArrow from={prevCustomers} to={company.customers} />
                    </div>
                    <div>
                      <p className="text-[10px] text-muted-foreground/70 mb-0.5">Headcount</p>
                      <DeltaArrow from={prevHeadcount} to={company.headcount} />
                    </div>
                    <div>
                      <p className="text-[10px] text-muted-foreground/70 mb-0.5">Burn</p>
                ...
                <div className="px-5 pb-5 pt-0 space-y-2 animate-fade-in-up">
                  {companyFlags.map(flag => (
                    <div key={flag.id} className="flex items-start gap-2.5 bg-amber-50/50 rounded-lg p-3">
                      ...
                      <div className="flex-1">
                        <p className="text-[13px] text-foreground">{flag.headline}</p>
                        <p className="text-[11px] text-muted-foreground mt-0.5">{flag.explanation}</p>
                      </div>
                      <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${flag.urgency === 'high' ? 'text-red-600 bg-red-50' : flag.urgency === 'medium' ? 'text-amber-600 bg-amber-50' : 'text-muted-foreground bg-muted/60'}`}>
                        {flag.urgency}
                      </span>
                    </div>
                  ))}
                ...
                <ol className="space-y-2.5">
                  {suggestedQuestions.map((item, i) => (
                    <li key={i} className="flex items-start gap-2.5 text-[13px]">
                      ...
                      <div className="flex-1">
                        <span contentEditable suppressContentEditableWarning className="outline-none focus:bg-blue-50 rounded px-1 -mx-1 block text-foreground">{item.q}</span>
                        <div className="mt-0.5">
                          <SourceTag label={item.src} />
                        </div>
                      </div>
                  ...
                  <li className="flex items-start gap-2.5 text-[13px] text-muted-foreground/70">
                    <span className="text-[11px] bg-muted rounded-full w-5 h-5 flex items-center justify-center shrink-0 mt-0.5">+</span>
                    <span contentEditable suppressContentEditableWarning className="outline-none focus:bg-muted/60 rounded px-1 -mx-1 flex-1 italic">Add your own question...</span>
                  </li>
                </ol>
              </div>
            )}
            ...
            {expandedSections.has('notes') && (
              <div className="px-5 pb-5 pt-0 animate-fade-in-up">
                <textarea
                  ...
                  className="w-full h-32 text-[13px] text-foreground bg-muted/60 border border-border/60 rounded-lg p-3 resize-y focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-200 placeholder:text-muted-foreground/50"
                />
        ...
        <div className="space-y-3" style={{ flexBasis: '35%', minWidth: 0 }}>
          ...
          <div className="bg-white rounded-xl border border-border/60 p-5">
            <p className="text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground/70 mb-3">Sources Used</p>
            <div className="space-y-1">
              {sourceDocuments.map((doc, i) => (
                <button
                  key={i}
                  onClick={() => navigate(`/company/${selectedCompany}`)}
                  className="w-full flex items-center gap-2.5 text-[12px] py-2 px-2.5 hover:bg-muted/60 rounded-lg transition-colors text-left group"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-muted-foreground/70">{doc.source} &middot; {doc.date}</p>
                  </div>
                  <ExternalLink className="w-3 h-3 text-muted-foreground/50 group-hover:text-blue-500 shrink-0 transition-colors" />
                </button>
              ))}
            </div>
          </div>
          ...
          {/* §B Recent Activity */}
          <div className="bg-white rounded-xl border border-border/60 p-5">
            <p className="text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground/70 mb-3">Recent Activity</p>
            {companyActivity.length === 0 ? (
              <p className="text-[12px] text-muted-foreground/70 italic">No recent activity</p>
            ) : (
              <div className="space-y-0">
                {companyActivity.map((event, i) => (
                  <button
                    key={event.id}
                    onClick={() => navigate(`/company/${selectedCompany}`)}
                    className="w-full flex items-start gap-2.5 py-2 px-2 hover:bg-muted/60 rounded-lg transition-colors text-left"
                  >
                    <div className="flex flex-col items-center mt-1">
                      <div className={`w-2 h-2 rounded-full ${event.severity === 'high' ? 'bg-red-400' : event.severity === 'medium' ? 'bg-amber-400' : 'bg-muted-foreground/50'}`} />
                      {i < companyActivity.length - 1 && <div className="w-px h-6 bg-secondary mt-1" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[12px] text-foreground font-medium truncate">{event.title}</p>
                      <p className="text-[11px] text-muted-foreground/70 truncate">{event.description}</p>
                      <p className="text-[10px] text-muted-foreground/50 mt-0.5">{new Date(event.timestamp).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}</p>
                    </div>
                  </button>
                ))}
              </div>
          ...
          {/* §C Open To-Dos */}
          <div className="bg-white rounded-xl border border-border/60 p-5">
            <p className="text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground/70 mb-3 flex items-center gap-1.5">
              <ListTodo className="w-3 h-3" /> Open To-Dos
            </p>
            {companyTodos.length === 0 ? (
              <p className="text-[12px] text-muted-foreground/70 italic">No pending tasks</p>
            ) : (
              <div className="space-y-1.5">
                {companyTodos.map(todo => (
                  <div key={todo.id} className="flex items-start gap-2 py-1.5">
                    <Circle className={`w-3.5 h-3.5 shrink-0 mt-0.5 ${todo.priority === 'high' ? 'text-red-400' : todo.priority === 'medium' ? 'text-amber-400' : 'text-muted-foreground/50'}`} />
                    <div className="flex-1 min-w-0">
                      <p className="text-[12px] text-foreground leading-snug">{todo.title}</p>
                      <p className="text-[10px] text-muted-foreground/70">Due {new Date(todo.dueDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {/* §D Quick Links */}
          <div className="bg-white rounded-xl border border-border/60 p-5">
            <p className="text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground/70 mb-3">Quick Links</p>
            <div className="space-y-1">
              <button
                onClick={() => navigate(`/company/${selectedCompany}`)}
                className="w-full flex items-center gap-2.5 text-[12px] text-foreground py-2 px-2.5 hover:bg-muted/60 rounded-lg transition-colors text-left"
              >
              ...
              <a
                ...
                className="flex items-center gap-2.5 text-[12px] text-foreground py-2 px-2.5 hover:bg-muted/60 rounded-lg transition-colors"
              >
              ...
              >
                ...
                {showPrevNotes ? <ChevronDown className="w-3 h-3 text-muted-foreground/50" /> : <ChevronRight className="w-3 h-3 text-muted-foreground/50" />}
              </button>
              {showPrevNotes && (
                <div className="ml-6 bg-muted/60 rounded-lg p-3 text-[12px] text-foreground/80 space-y-1.5">
                  <p className="text-[10px] text-muted-foreground/70 font-medium">Board #3 — 15 Dec 2025</p>
                  ...
                  <p className="text-[10px] text-muted-foreground/70 font-medium mt-2">Board #2 — 18 Sep 2025</p>
